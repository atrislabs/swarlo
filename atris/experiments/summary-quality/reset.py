"""Restore the Swarlo summary target to its baseline."""

from pathlib import Path


TARGET = Path(__file__).resolve().parents[3] / "swarlo" / "sqlite_backend.py"
START = "    async def summarize_for_member(self, hub_id: str, member_id: str, limit: int = 10) -> str:\n"
END = "\n    # ── DAG"
BASELINE = '''    async def summarize_for_member(self, hub_id: str, member_id: str, limit: int = 10) -> str:
...
TARGET.write_text(text[:start] + BASELINE + text[end:], encoding="utf-8")

print("reset summary-quality target to baseline")
