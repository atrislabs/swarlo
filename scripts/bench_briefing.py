...
def run_bench(n_total: int, k_relevant: int, iters: int, seed: int) -> dict:
    ...
    try:
        ...
        metrics_by_scorer: dict[str, list[dict]] = {"regex": [], "tfidf": []}
...
def main():
    ...
    header = f"{'metric':14s}  {'regex (v1)':>12s}  {'tfidf (v2)':>12s}  {'Δ':>10s}"
    print(header)
    print("-" * 72)
    for field, label in key_metrics:
        ...
        delta = t - r
        if "latency" in field:
            print(f"{label:14s}  {r:12.2f}  {t:12.2f}  {delta:+10.2f}")
        else:
            print(f"{label:14s}  {r:12.3f}  {t:12.3f}  {delta:+10.3f}")
