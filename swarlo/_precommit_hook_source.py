...
def main() -> int:
    ...
    # Build {file_path: claim_dict} for files claimed by OTHER members
    others_claims: dict[str, dict] = {}
    for c in claims:
        if c.get("member_id") != member_id and c.get("file_path"):
            others_claims[c["file_path"]] = c

    if not others_claims:
        return 0

    # Check intersection with staged files
    conflicts = []
    for f in staged:
        if f in others_claims:
            conflicts.append((f, others_claims[f]))
